...
export default function () {
  ...
  const topList = [{name:'音樂館',flag:false},{name:'我的音樂',flag:false},{name:'开放平台',flag:false}]
  ...
  const qrcode = ()=>{
    QrKey().then(({data:{data}}) => {
      qrcheck(data.unikey)
      QrCreate({key:data.unikey}).then(({data:{data}}) => {
        console.log(data,'结果');
  ...
  function Phone() {
   return (
			<div className={styles.loginBox}>
				<div>
					<Input placeholder="请输入手机号" value={PhonePass.phone} onChange={e=>PhoneInput(e)}  ></Input>
				</div>
				<div>
					<Input type="password" placeholder="请输入密码" value={PhonePass.password}></Input>
				</div>
				<Button type="primary" onClick={userLogin}>授权登录</Button>
			</div>
		);
  }
  // 二维码登录
      ...
      <div className={styles.homeTop}>
        ...
        <div>
            ...
            title.map((item,index)=>(
              <div className={item.flag?styles.ckdiv:null}  key={item.name} onClick={()=>selectTop(index)}>{item.name}</div>
            ))
          }
				</div>
        ...
        <div className={styles.homeTopSet}>
          <div>
            <Search placeholder="请输入您要查询的歌曲" enterButton  ref={input} size='large' />
          </div>
          <div className={styles.userHeadImg}>
            <img onClick={showModal} src="https://thirdqq.qlogo.cn/g?b=sdk&k=EsvwYj7dS8AoN2x5EwNvvg&s=140&t=1626453179" alt=""/>
          </div>
        </div>
			</div>
